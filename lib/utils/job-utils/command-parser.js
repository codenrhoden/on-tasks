...
function commandParserFactory(Logger, Promise, _) {
    ...
    var matchParsers = {};
    matchParsers.freeMemoryMb = {
	regex: /^echo.*free\s-m.*$/,
        parsefunction: function (data) {
            if (data.error) {
                return Promise.resolve({ source: 'total-memory-mb', error: data.error });
            }
            try {
                var parsed = JSON.parse(data.stdout);
                return Promise.resolve({ data: parsed, source: 'total-memory-mb', store: true });
            } catch (e) {
                return Promise.resolve({ source: 'total-memory-mb', error: e });
            }
	}
    }
